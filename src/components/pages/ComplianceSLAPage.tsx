...
import { exportVulnerabilities } from '../../utils/exportUtils';
import { 
  Table, 
  TableBody, 
  TableCell, 
...
export function ComplianceSLAPage() {
  ...
  const domainCompliance = mockDomains.map(domain => {
  ...
  const getAgingData = () => {
    ...
    return Object.entries(aging).map(([range, count]) => ({
      range,
      count,
      percentage: (count / mockVulnerabilities.filter(v => v.status === 'open').length) * 100
    }));
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => {
                  actions.addNotification({
                    ...
                    message: 'Advanced filtering options would be displayed here'
                  });
                }}
              >
              ...
              <Button
                ...
                onClick={() => {
                  try {
                    exportVulnerabilities(state.vulnerabilities, { format: 'csv', filename: 'sla_compliance_report.csv' });
                    actions.addNotification({
                      type: 'success',
                      title: 'Export Successful',
                      message: 'SLA compliance report exported successfully'
                    });
                  } catch (error) {
                    actions.addNotification({
                      type: 'error',
        ...
        <Card className="bg-gradient-to-r from-red-50 to-red-100 dark:from-red-950 dark:to-red-900 border-red-200">
          <CardContent className="p-4">
            ...
            <Button
              ...
              onClick={() => {
                const overdueVulns = state.vulnerabilities.filter(v =>
                  new Date(v.slaDue) < new Date() && v.status === 'open'
                );
                actions.addNotification({
                  type: 'warning',
                  title: 'Overdue Items Review',
                  message: `Found ${overdueVulns.length} overdue vulnerabilities requiring immediate attention`
                });
              }}
            >
        ...
        <Card className="bg-gradient-to-r from-orange-50 to-orange-100 dark:from-orange-950 dark:to-orange-900 border-orange-200">
          <CardContent className="p-4">
            ...
            <Button
              ...
              variant="outline"
              onClick={() => {
                const atRiskVulns = state.vulnerabilities.filter(v => {
                  const dueDate = new Date(v.slaDue);
                  const now = new Date();
                  const hoursUntilDue = (dueDate.getTime() - now.getTime()) / (1000 * 60 * 60);
                  return hoursUntilDue <= 24 && hoursUntilDue > 0 && v.status === 'open';
                });
                actions.addNotification({
                  type: 'info',
                  title: 'Action Prioritization',
                  message: `${atRiskVulns.length} vulnerabilities have been prioritized for immediate action`
                });
              }}
            >
              Prioritize Actions
        ...
        <Card className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900 border-blue-200">
          <CardContent className="p-4">
            ...
            <Button
              ...
              onClick={async () => {
                await actions.generateReport('SLA Compliance Report', {
                  includeMetrics: true,
                  includeCharts: true,
                  format: 'pdf'
                });
