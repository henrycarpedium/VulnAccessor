...
export function DashboardPage() {
  ...
  // Calculate real-time global metrics from current state
  const globalMetrics = {
    ...
    averageRiskScore: state.domains.reduce((acc, d) => acc + d.riskScore, 0) / state.domains.length,
    slaCompliance: state.domains.reduce((acc, d) => acc + d.slaCompliance, 0) / state.domains.length,
    mttd: 2.4,
    mttr: 18.7
  };

  const handleQuickScan = () => {
    const highRiskDomain = state.domains.find(d => d.riskScore >= 8.0);
    if (highRiskDomain) {
      actions.startScan(highRiskDomain.id);
    } else {
      actions.addNotification({
        type: 'info',
        title: 'No High-Risk Domains',
        message: 'All domains are currently low risk. Manual scan selection required.'
      });
    }
  };

  const handleGenerateReport = () => {
    actions.generateReport('Executive Summary', {
      dateRange: '30d',
      domains: state.domains.map(d => d.id),
      includeCharts: true
    });
  };
