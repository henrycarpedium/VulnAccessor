...
export function MLInsightsPage() {
  ...
  const handleSendMessage = () => {
    ...
    if (input.includes('spike') || input.includes('increase')) {
      aiResponse = 'The risk spike in app.company.com was caused by 3 new critical vulnerabilities discovered on January 10th. Our ML model detected unusual patterns in the codebase changes that preceded these vulnerabilities. The confidence level for this analysis is 94%.';
    } else if (input.includes('threat') || input.includes('pattern')) {
      aiResponse = 'Key threat patterns detected: 1) SQL injection vulnerabilities cluster around login systems (67% correlation), 2) XSS issues tend to spike after feature deployments (82% correlation), 3) Access control issues show seasonal patterns, typically increasing during team scaling periods.';
    } else if (input.includes('prediction') || input.includes('future')) {
      aiResponse = 'Based on current trends, I predict app.company.com risk score will reach 9.5 within 2 days if no action is taken. Recommended immediate actions: 1) Patch SQL injection in login form, 2) Review access controls, 3) Schedule emergency security review.';
    } else {
      aiResponse = 'I analyzed your query and found relevant patterns in our security data. Could you be more specific about which domain or vulnerability type you\'re interested in?';
    }

    newMessages.push({ type: 'assistant', message: aiResponse });
