...
export function WorkflowVisualizerPage() {
  ...
  const [selectedDomain, setSelectedDomain] = useState(state.domains[0]?.name || 'app.company.com');
      ...
      interval = setInterval(() => {
        ...
        setCurrentStep(prev => {
          if (prev >= steps.length - 1) {
            setIsPlaying(false);
            // Workflow completed - add notification
            actions.addNotification({
              type: 'success',
              title: 'Workflow Complete',
              message: `VAPT workflow completed for ${selectedDomain}`
            });
            return prev;
          }
          return prev + 1;
