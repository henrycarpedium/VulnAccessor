...
import { mockRiskTimeline } from '../../data/mockData';

export function DomainDetailsPage() {
  const { state, actions } = useApp();
  const [selectedDomain, setSelectedDomain] = useState(state.domains[0]?.id || '');
  
  const currentDomain = state.domains.find(d => d.id === selectedDomain) || state.domains[0];
  const domainVulns = state.vulnerabilities.filter(v => v.domain === currentDomain?.name);
  const domainRiskHistory = mockRiskTimeline.filter(item => item.domain === currentDomain?.name);
  const domainScans = state.scans.filter(s => s.domain === currentDomain?.name);

  if (!currentDomain) {
    return (
      <div className="p-6">
        <h1 className="text-3xl font-bold mb-4">Domain Details</h1>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-4">
        <Button 
          ...
          onClick={() => actions.startScan(currentDomain.id)}
          disabled={state.loading || currentDomain.status === 'scanning'}
        >
          <Activity className="h-4 w-4" />
          {currentDomain.status === 'scanning' ? 'Scanning...' : 'Run New Scan'}
        </Button>
        <Button 
          variant="outline" 
          className="flex items-center gap-2"
          onClick={() => actions.generateReport('Domain Security Report', {
        ...
        <Button 
          ...
          onClick={() => actions.addNotification({
            type: 'info',
            title: 'Schedule Scan',
            message: 'Scan scheduling feature would be implemented here.'
          })}
        >
